...
void CI_CustomMain(void)
{
    ...
    while(size >= 0)
    {
        ...
        if (size > 0)
        {
            OS_printf("ci_custom: process data\n");

            /* Process TC Frame */
            if ((g_CI_CustomData.buffer[0]==0x18) && (g_CI_CustomData.buffer[1]==0x84) && (g_CI_CustomData.buffer[2]==0xC0) && 
                (g_CI_CustomData.buffer[3]==0x00) && (g_CI_CustomData.buffer[4]==0x00) && (g_CI_CustomData.buffer[5]==0x12) && 
                (g_CI_CustomData.buffer[6]==0x00) && (g_CI_CustomData.buffer[7]==0x06)) 
            {
                /* Pass TO */
                OS_printf("ci_custom: Pass TO \n");
            }
            else
            {             
                /* Process Security - START */
                result = Crypto_TC_ProcessSecurity((char *) &g_CI_CustomData.buffer, &size);
                if (result != OS_SUCCESS)
                {
                    /* TODO: Crypto Process Security Error */
                    OS_printf("ci_custom: Crypto_TC_ProcessSecurity Error!\n");
                }
                else
                {
                    if (size == 0)
                    {
                        /* Do nothing */
                        OS_printf("ci_custom: size = 0 \n");
                    }
                    else
                    {
                        OS_printf("ci_custom: size = %d, g_CI_CustomData.buffer[0] = 0x%02x, g_CI_CustomData.buffer[1] = 0x%02x \n", size, g_CI_CustomData.buffer[0], g_CI_CustomData.buffer[1]);
                        ...
                        {
                            OS_printf("ci_custom: sending message\n");
                            CFE_SB_SendMsg(sbMsg);
                        }
                    }
                }
                /* Process Security - END */
            }
        }
    }
